...
148 test files across stores (8), services (79), utils (18), components (35), constants (4), router (1), hooks (3), and config (1).

## Database

SQLite via Tauri SQL plugin. 29 migrations (version-tracked in `_migrations` table, transactional). Custom `splitStatements()` handles BEGIN...END blocks in triggers.
