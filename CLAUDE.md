...
- **Send-as aliases**: Fetched from Gmail `/settings/sendAs` on account init (Gmail only) — needs the `gmail.settings.basic` scope, so accounts authorized before it was requested must be re-authorized or the call 403s and no aliases load. Aliases appear as selectable identities under their account in `AccountSwitcher`; the choice is stored as `accountStore.activeAliasEmail` (persisted `active_alias_email`) and the composer defaults `From` to it. `FromSelector` still allows a per-message override. Gmail only accepts addresses verified as send-as on the account — Velo cannot add them
