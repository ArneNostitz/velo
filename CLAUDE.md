...
- `email/` — ThreadView, ThreadCard, MessageItem, EmailRenderer, ActionBar, AttachmentList, SnoozeDialog, ContactSidebar, FollowUpDialog, InlineAttachmentPreview, InlineReply, SmartReplySuggestions, ThreadSummary, AuthBadge, AuthWarningBanner, PhishingBanner, ReadReceiptBanner, ThreadFilesSection, LinkConfirmDialog, CategoryTabs, MoveToFolderDialog
...
139 test files across stores (8), services (74), utils (15), components (33), constants (4), router (1), hooks (3), and config (1).
...
- **Sync indicator**: `uiStore.syncState`/`syncMessage` drive a ring around the account avatar in `AccountSwitcher` (spinning while syncing, red on failure). There is no bottom status bar
