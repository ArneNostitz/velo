...
- `ui/` — EmptyState, Skeleton, ContextMenu, ContextMenuPortal, OfflineBanner, Tooltip, Spinner, ToastHost, illustrations/ (InboxClearIllustration, NoAccountIllustration, NoSearchResultsIllustration, ReadingPaneIllustration, GenericEmptyIllustration)
...
155 test files across stores (9), services (82), utils (19), components (37), constants (4), router (1), hooks (3), and config (1).

## Database

SQLite via Tauri SQL plugin. 32 migrations (version-tracked in `_migrations` table, transactional). Custom `splitStatements()` handles BEGIN...END blocks in triggers, and skips `--`/`/* */` comments and string literals when looking for the statement separator — a semicolon inside an explanatory comment once split the statement after it, which failed the migration and left the app with no accounts. A test asserts every migration's statements start with a SQL verb once comments are stripped.
...
- **One-time codes & sign-in links**: `utils/otpDetector.ts` finds a login code only when a code *word* sits within 60 chars of a code-shaped token — a false positive would silently overwrite the clipboard, so years, repeated-digit placeholders and bare order numbers are rejected by test. `services/otp/otpManager.ts` copies it (via the Rust `clipboard-manager` plugin — `navigator.clipboard` needs the document focused, and the whole point is that the user is in another app) and notifies. Only messages under 10 minutes old are acted on, or a first sync would copy an ancient code and fire a notification per historical login. A magic link is taken from the *anchors*, so the link's own words qualify it, and never from the unsubscribe footer. A mail that carries both a code and a link announces both. **Desktop notifications carry no buttons**: `tauri-plugin-notification` on macOS hands the text to `notify-rust` (legacy `NSUserNotification`, no categories), and `registerActionTypes`/`onAction` are mobile-only no-ops on desktop — so the existing Reply/Archive actions never worked there either, and the Alerts-vs-Banners setting is irrelevant because nothing is attached to hover for. The buttons live where they can: an in-app toast (`notify(..., actions)` — Copy code / Open sign-in link, kept until used) and `OneTimeCodeBanner` on the open message. Real notification buttons would need a native `UNUserNotificationCenter` path in Rust. Keywords match as **whole words only** — `otp` inside a hex token in a tracking URL once qualified the nearest number, a Berlin postal code — and the weak bare-`code` path additionally requires the digits to stand alone on their line (the shape a login mail uses), while a `, 12345 Town` pattern is rejected as an address regardless
...
- **Rejoining split threads**: `services/threading/threadLinker.ts` runs each sync and folds threads the provider split, reusing `merged_into` so every join is reversible. Two rules: `linkSplitThreads()` follows `In-Reply-To` to a message in another thread (factual, not a guess — this is what the parser fix unlocked), and `linkThreadsBySubject()` matches a normalised subject plus a shared correspondent within 30 days. `normalizeSubject()` strips stacked prefixes in several languages and the stray separator they leave behind ("AW: : Subject"); generic or very short subjects never qualify. **The subject rule requires a real exchange on both sides** (`qualifiesForSubjectMerge`): the user *and* someone else in each thread, and the shared correspondent must be someone other than the user. Each half exists because it was wrong once: a shared-correspondent test alone folded 243 same-subject notifications (v1); "the user wrote in it" alone folded them again, because a magic-link mail arrives *from the user's own address* — `me:` in the list, `SENT` on the thread (v2); and counting the user as the shared peer matched any two of their exchanges (v3). Migration 32 undid every `subject`-tagged merge. `threads.merged_by` records `manual`/`header`/`subject` so a rule that turns out wrong can be undone for exactly its own merges; migration 31 separated every merge with no reason on record
...
- **Fail loudly**: every failure a person could act on goes through `reportError(title, err, action?)` from `stores/toastStore.ts`, never a bare `console.error` — for a long time a stopped sync, a refused IDLE watcher or a thrown migration went to a console nobody was reading and the app just went quietly strange. `ToastHost` (mounted in `App`, top-right under the title bar so it never fights the undo-send/update toasts at the bottom) shows them; errors are `role="alert"` and **stay until dismissed**, identical title+detail collapses into one toast so a 15s retry cannot build a wall, and an optional action ("Retry", "Reconnect") sits on the toast. `ErrorBoundary`, the window `error`/`unhandledrejection` events, sync failure, migration failure, send failures, IDLE refusals and the sync-side catches all route through it. `notify(kind, title)` is the non-error sibling and fades
- **Spinners**: anything connecting, syncing, sending or refreshing shows `ui/Spinner` while it does — a control waiting on a server that looks identical to an idle one reads first as "nothing is happening" and then as "broken". Used on Reconnect, Re-authorize and Resync, and in place of the status dot while an account is `connecting`
- **Delivery status lives under Accounts**: per-account instant-delivery state, the Reconnect button and the master toggle sit in Settings → Accounts with the account they belong to, not under Notifications
- **Tooltips**: use `ui/Tooltip` (`<Tooltip content=… placement=…><button/></Tooltip>`), never the `title` attribute for anything a person is meant to read — the system tooltip waits about a second and on macOS sometimes never comes. `Tooltip` shows at 0ms, opens on focus as well as hover, closes on Escape, sets `aria-describedby`, portals to `body` so no ancestor overflow clips it, flips sides at a window edge, and calls the child's own handlers through. It is decoration: anything vital still belongs in the element itself
- **Instant-delivery status**: `stores/idleStatusStore.ts` holds `off | connecting | connected | failed` per account, driven by what the Rust watcher *reports*, not by what was requested — the setting being on says nothing about whether a server actually accepted the connection. Rust emits `velo-idle-status` (`connected` on INBOX select, `disconnected` whenever a session ends); `idleManager` maps a drop to `connecting` (the loop is already reconnecting) and only a refusal to `failed`, keeping the server's reason for `explainIdleFailure()`. Shown in the account-avatar tooltip (a dot + one line, summarised across accounts in the unified inbox) and as per-account rows under Settings → Notifications → Delivery, with a Reconnect button
- **IMAP IDLE**: `src-tauri/src/imap/idle.rs` holds one IDLE connection per account and emits `velo-idle-activity`; `services/imap/idleManager.ts` starts the watchers and turns that event into a normal `syncAccount()`. The push is only a doorbell — the payload is ignored and the existing sync finds out what changed, so a Gmail account keeps the Gmail API as its source of truth and merely stops waiting. It works behind NAT because the connection is dialled *out*; nothing is hosted and no endpoint is exposed, which is why this replaced the Pub/Sub relay idea. Gmail IMAP needs the `https://mail.google.com/` scope, so accounts authorised before it was requested must be re-authorised — they fail the IMAP auth and quietly stay on the timer. IDLE is renewed every 25 minutes (RFC 2177 says under 29) and the 60s poll stays as the safety net for sleep, network changes and servers that refuse. Two traps already paid for: the Gmail config in `idleManager` is hand-built and bypasses `mapSecurity()`, so its `security` must be the value Rust matches on — `"tls"`, not the `"ssl"` the account form stores (every watcher was refused with "Unknown security mode: ssl" and the 15s retry made it look like a flaky connection); and `is_permanent()` in `idle.rs` stops the loop on errors retrying cannot fix (bad security mode, refused login) so they surface as `failed` with a reason instead of spinning as `connecting` — a DNS lookup failure is deliberately *not* on that list, it is a Wi-Fi flap and once stopped a watcher for good on one blip. **The direct-TLS path must read the server greeting before `Client::new`** (`connect_inner` in `client.rs`): `Client::new` does not consume it, so `* OK Gimap ready` was taken as the reply to `AUTHENTICATE`, the XOAUTH2 handshake waited for a `+` that had already gone by, and every Gmail account hit the 30s auth timeout — the STARTTLS path always read its greeting, and Gmail is the only direct-TLS user, which is why nothing else noticed. Reconnect is per account (`reconnectAccount(id)`), never all-at-once. `ui/Spinner` uses `.spinner-motion`, which the reduce-motion rules slow rather than freeze — a frozen spinner reads as a stuck control
